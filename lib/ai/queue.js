...
function sectionsToHtml(sections) {
  ...
  return sections
    .map((s) => `<h2 id="sec-${s.anchor_id || ''}">${s.title}</h2>${s.html || ''}`)
    .join('\n');
}
...
async function processGenerate(job) {
  ...
  try {
    ...
    const bioData = bio.data || null;

    if (!bioData || bioData.error === 'not_russian') {
      await db.generationLog.update({
        where: { id: log.id },
        data: {
          status: 'FAILED',
    ...
    const intro = bioData.intro || '';
    const sections = Array.isArray(bioData.sections) ? bioData.sections : [];
    const profession = bioData.profession || null;
    const bioHtmlFallback = sectionsToHtml(sections);

    const bioWithLinks = await addInternalLinks(bioHtmlFallback, null);

    const seo = await deepseekGenerate('seo_meta', { name, bio: bioHtmlFallback });
    const short = await deepseekGenerate('short_bio', { name, bio: bioHtmlFallback });
    ...
    const person = await db.person.upsert({
      ...
      create: {
        ...
        bio_short: short.text?.trim() || intro.slice(0, 280),
        sections,
        profession,
        birth_date: parseRuDate(bioData.birth_date),
        death_date: parseRuDate(bioData.death_date),
        birth_place: bioData.birth_place || null,
        zodiac: bioData.zodiac || null,
        social_links: Array.isArray(bioData.social_links) ? bioData.social_links : [],
...
async function processUpdate(job) {
  ...
  const bioData = bio.data || null;
  if (!bioData || bioData.error === 'not_russian') {
    return { skipped: true, reason: 'not_russian_or_parse_error' };
  }

  const sections = Array.isArray(bioData.sections) ? bioData.sections : [];
  const bioHtml = sectionsToHtml(sections);
  const bioWithLinks = await addInternalLinks(bioHtml, personId);

  await db.person.update({
    where: { id: personId },
    data: {
      intro: bioData.intro || person.intro,
      sections,
      bio_full: bioWithLinks,
      ai_model: bio.model,
